...
namespace SistemaRestaurante.controlador
{
    class ControlUsuario
    {
        ...
        public string inicioSesion(usuario _user)
        {
            ...
            Comando.Parameters.AddWithValue("@Usuario", _user.Nombe);
            Comando.Parameters.AddWithValue("@Password", _user.Pass);
        ...
        public Boolean eliminarUsuario(usuario _userr)
        {
            int eliminar = 0;
            bool respuesta = false;
            SqlCommand comando = new SqlCommand("sp_eliminarUsuario", conexion.openDataBase());
            comando.CommandType = System.Data.CommandType.StoredProcedure;
            comando.Parameters.AddWithValue("@nombreUsuario", _userr.Nombe);
        ...
        public List<usuario> seleccionarUsuarios()
        {
            ...
            try
            {
                ...
                while (lector.Read())
                {
                    usuario _personaal = new usuario();
                    _personaal.Id = int.Parse(lector[0].ToString());
                    _personaal.Nombe = lector[1].ToString();
                    listPersonal.Add(_personaal);
                }
